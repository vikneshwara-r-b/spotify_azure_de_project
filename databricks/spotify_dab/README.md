...
variables:
  ...
  adf_service_principal_id:
    description: "ADF Managed Identity Application ID"
    default: ${env.ADF_SERVICE_PRINCIPAL_ID}
...
**Variables** (Required):
- `DATABRICKS_HOST`: Workspace URL (required for deployment)
- `ADLS_STORAGE_CONTAINER_NAME`: Storage account name
- `USER_EMAIL`: User for permissions (optional, for prod)
- `ADF_SERVICE_PRINCIPAL_ID`: ADF MI principal ID (optional, for permissions)
...
export ADLS_STORAGE_CONTAINER_NAME="adlsspotify001"
...
cd spotify_azure_de_project/databricks/spotify_dab

# 2. Install dependencies
uv sync

# 3. Develop notebooks in Databricks workspace
...
# Or: databricks bundle run --target dev
...
|----------|----------|-------------|----------|
| `DATABRICKS_HOST` | ✅ Yes | Workspace URL | `terraform output databricks_workspace_url` |
| `ADLS_STORAGE_CONTAINER_NAME` | ✅ Yes | Storage account name | `terraform output storage_account_name` |
| `USER_EMAIL` | ❌ Optional | User for permissions | Your email |
| `ADF_SERVICE_PRINCIPAL_ID` | ❌ Optional | ADF MI principal ID | `terraform output adf_managed_identity_application_id` |
